<template>
  <view class="p-24rpx pb-32rpx">
    <view class="mb-16rpx flex items-center justify-between">
      <text class="yd-text-main text-28rpx font-semibold">
        联系人
      </text>
      <text
        v-if="hasAccessByCodes(['hrm:employee:update'])"
        class="yd-text-link text-28rpx"
        @click="openForm()"
      >
        新增
      </text>
    </view>
    <view v-if="!list.length" class="yd-text-hint py-40rpx text-center text-28rpx">
      暂无联系人
    </view>
    <view
      v-for="item in list"
      :key="item.id"
      class="mb-24rpx rounded-12rpx bg-white p-24rpx shadow-sm"
    >
      <view class="yd-text-main mb-12rpx text-30rpx font-semibold">
        {{ item.name || '-' }}
      </view>
      <view class="yd-text-sub mb-8rpx text-26rpx">
        关系：{{ item.relation || '-' }}
      </view>
      <view class="yd-text-sub mb-8rpx text-26rpx">
        电话：{{ item.phone || '-' }}
      </view>
      <view class="yd-text-sub mb-8rpx text-26rpx">
        工作单位：{{ item.workUnit || '-' }}
      </view>
      <view class="yd-text-sub mb-8rpx text-26rpx">
        职务：{{ item.postName || '-' }}
      </view>
      <view class="yd-text-sub mb-16rpx text-26rpx">
        地址：{{ item.address || '-' }}
      </view>
      <view
        v-if="hasAccessByCodes(['hrm:employee:update']) || hasAccessByCodes(['hrm:employee:delete'])"
        class="yd-border-light flex gap-32rpx border-t pt-16rpx"
      >
        <text
          v-if="hasAccessByCodes(['hrm:employee:update'])"
          class="yd-text-link text-28rpx"
          @click="openForm(item)"
        >
          编辑
        </text>
        <text
          v-if="hasAccessByCodes(['hrm:employee:delete'])"
          class="yd-text-danger text-28rpx"
          @click="handleDelete(item)"
        >
          删除
        </text>
      </view>
    </view>
    <ContactForm ref="formRef" @success="getList" />
  </view>
</template>

<script lang="ts" setup>
import type { EmployeeContact } from '@/api/hrm/employee/contact'
import { useDialog } from '@wot-ui/ui/components/wd-dialog'
import { useToast } from '@wot-ui/ui/components/wd-toast'
import { onMounted, ref, watch } from 'vue'
import {
  deleteEmployeeContact,
  getEmployeeContactList,
} from '@/api/hrm/employee/contact'
import { useAccess } from '@/hooks/useAccess'
import ContactForm from './contact-form.vue'

const props = defineProps<{
  employeeId: number
}>()

const { hasAccessByCodes } = useAccess()
const dialog = useDialog()
const toast = useToast()
const list = ref<EmployeeContact[]>([]) // 联系人列表
const formRef = ref<InstanceType<typeof ContactForm>>() // 表单

/** 加载联系人 */
async function getList() {
  if (!props.employeeId) {
    list.value = []
    return
  }
  list.value = await getEmployeeContactList(props.employeeId)
}

/** 打开表单 */
function openForm(row?: EmployeeContact) {
  formRef.value?.open(props.employeeId, row)
}

/** 删除联系人 */
async function handleDelete(item: EmployeeContact) {
  if (!item.id) {
    return
  }
  try {
    await dialog.confirm({
      title: '提示',
      msg: `确认删除联系人“${item.name || ''}”吗？`,
    })
  } catch {
    return
  }
  await deleteEmployeeContact(item.id)
  toast.success('删除成功')
  await getList()
}

watch(() => props.employeeId, () => getList())

onMounted(() => {
  getList()
})

defineExpose({ getList, openAdd: () => openForm() })
</script>
