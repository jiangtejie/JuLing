    ...
    <view class="bg-white px-24rpx pb-32rpx pt-24rpx">
      ...
      <wd-form ref="formRef" :model="formData" :schema="formSchema">
        <wd-cell-group border>
          ...
          <ChannelFormPicker v-model="formData.channelId" prop="channelId" :clearable="false" />
        </wd-cell-group>
      </wd-form>
...
const formSchema = createFormSchema({
  channelId: [{ required: true, message: '招聘渠道不能为空' }],
})
...
function open(ids: number[]) {
  ...
  formData.value = { channelId: undefined }
  visible.value = true
}
...
async function handleSubmit() {
  ...
  if (!valid || formData.value.channelId == null) {
    return
  }
  formLoading.value = true
  try {
    const hasSuccess = await executeBatch(
      candidateIds.value.map(id => updateRecruitCandidateChannel({
        id,
