...
export function getKnowledgeLibraryRecycleList() {
  return http.get<KnowledgeRecycle[]>('/pms/kb/recycle/library-list')
}

/** 查询知识库内容回收站列表 */
export function getKnowledgeContentRecycleList(libraryId: number) {
  return http.get<KnowledgeRecycle[]>('/pms/kb/recycle/content-list', { libraryId })
}

/** 查询知识库回收站对象详情及级联内容 */
export function getKnowledgeContentRecycleDetail(id: number) {
  return http.get<KnowledgeRecycleDetail>('/pms/kb/recycle/content-detail', { id })
}

/** 预览知识库回收站内容 */
export function getKnowledgeContentRecyclePreview(id: number, entityId?: number) {
  return http.get<KnowledgeRecyclePreview>('/pms/kb/recycle/content-preview', { id, entityId })
}

/** 恢复回收站记录 */
export function restoreKnowledgeRecycle(id: number) {
  return http.put<boolean>('/pms/kb/recycle/restore', undefined, { id })
}

/** 彻底删除回收站记录 */
export function permanentDeleteKnowledgeRecycle(id: number) {
  return http.delete<boolean>('/pms/kb/recycle/permanent-delete', undefined, { id })
}
