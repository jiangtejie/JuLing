<template>
  <view class="yd-page-container">
    ...
    <!-- 底部操作按钮 -->
    <view v-if="hasAccessByCodes(['erp:supplier:update']) || hasAccessByCodes(['erp:supplier:delete'])" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button v-if="hasAccessByCodes(['erp:supplier:update'])" class="flex-1" type="warning" @click="handleEdit">
          编辑
        </wd-button>
        <wd-button v-if="hasAccessByCodes(['erp:supplier:delete'])" class="flex-1" type="danger" :loading="deleting" @click="handleDelete">
          删除
        </wd-button>
      </view>
    </view>
