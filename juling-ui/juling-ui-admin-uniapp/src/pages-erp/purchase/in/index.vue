<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="采购入库" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />
        ...
        >
          <view>
            <view class="p-24rpx">
              ...
              <view class="yd-text-sub mb-12rpx text-28rpx">
                <text class="yd-text-hint mr-8rpx">入库时间：</text>{{ formatDateTime(item.inTime) || '-' }}
              </view>
              <view class="yd-text-sub mb-12rpx flex text-28rpx">
                ...
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">已付：</text>{{ formatMoney(item.paymentPrice) }}
                </view>
              </view>
              <view class="yd-text-sub flex text-28rpx">
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">应付：</text>{{ formatMoney(item.totalPrice) }}
                </view>
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">未付：</text>{{ formatMoney(getUnpaidPrice(item)) }}
                </view>
              </view>
            </view>
          </view>
...
const list = ref<PurchaseIn[]>([]) // 列表数据
...
/** 计算未付款金额 */
function getUnpaidPrice(item: PurchaseIn) {
  return Number(item.totalPrice || 0) - Number(item.paymentPrice || 0)
}

/** 查询列表 */
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getPurchaseInPage({ ...queryParams.value, pageNo, pageSize })
...
/** 新增 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-erp/purchase/in/form/index' })
}

/** 查看详情 */
function handleDetail(item: PurchaseIn) {
  uni.navigateTo({ url: `/pages-erp/purchase/in/detail/index?id=${item.id}` })
}

/** 初始化 */
onMounted(() => {
  uni.$on('erp:purchase-in:reload', reload)
})

/** 解绑页面事件 */
onUnload(() => {
  uni.$off('erp:purchase-in:reload', reload)
})
</script>
