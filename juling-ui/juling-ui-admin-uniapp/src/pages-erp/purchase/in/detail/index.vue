<template>
  <view class="yd-page-container">
        ...
        <view class="px-24rpx">
          ...
          >
            ...
            <view class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">仓库：</text>
            ...
            <view v-if="item.totalCount != null" class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">原数量：</text>
              <text class="min-w-0 flex-1">{{ formatCount(item.totalCount) }}</text>
            </view>
            <view v-if="item.inCount != null" class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">已入库：</text>
    ...
    <!-- 底部操作按钮 -->
    <view v-if="canUpdate || canUpdateStatus || hasAccessByCodes(['erp:purchase-in:delete'])" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button
          v-if="hasAccessByCodes(['erp:purchase-in:delete'])"
...
const formData = ref<PurchaseIn>() // 详情数据
...
const canUpdate = computed(() => formData.value?.status !== ErpAuditStatusEnum.AUDITED && hasAccessByCodes(['erp:purchase-in:update']))
const canUpdateStatus = computed(() => hasAccessByCodes(['erp:purchase-in:update-status']) && (formData.value?.status === ErpAuditStatusEnum.UNAUDITED || formData.value?.status === ErpAuditStatusEnum.AUDITED))
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await buildErpDocumentDetail(await getPurchaseIn(props.id), 'purchase-in')
  } finally {
    toast.close()
  }
}
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updatePurchaseInStatus(props.id, status)
