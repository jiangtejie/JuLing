      ...
      <view v-if="items.length > 0" class="mt-24rpx">
        ...
        <view class="px-24rpx">
          ...
          >
            ...
            <view v-if="item.returnCount != null" class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">已退货：</text>
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await buildErpDocumentDetail(await getSaleReturn(props.id), 'sale-return') as SaleReturn
  } finally {
    toast.close()
  }
}
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updateSaleReturnStatus(props.id, status)
