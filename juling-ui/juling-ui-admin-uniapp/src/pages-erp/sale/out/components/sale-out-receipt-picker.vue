  ...
  >
    <view class="yd-bg-page h-full flex flex-col">
      ...
      <view class="bg-white px-24rpx pb-20rpx">
        <wd-input v-model="queryParams.no" placeholder="请输入销售出库单号" clearable />
        <ProductFormPicker v-model="queryParams.productId" label="" placeholder="请选择产品" class="mt-12rpx" />
        <yd-search-date-range v-model="queryParams.time" class="mt-12rpx" label="出库时间" />
        ...
        <view class="p-24rpx">
          ...
          >
            ...
            <view class="yd-text-sub flex text-26rpx">
              <view class="flex-1">
                <text class="yd-text-hint mr-8rpx">应收金额：</text>{{ formatMoney(item.totalPrice) }}
              </view>
              <view class="flex-1">
                <text class="yd-text-hint mr-8rpx">已收金额：</text>{{ formatMoney(item.receiptPrice) }}
              </view>
            </view>
          </view>
        </view>
...
const emit = defineEmits<{
  success: [rows: SaleOut[]]
}>()

const visible = ref(false) // 弹窗显示状态
const customerId = ref<number>() // 客户编号
const list = ref<SaleOut[]>([]) // 可收款销售出库列表
const selectedRows = ref<SaleOut[]>([]) // 已选销售出库
const pagingRef = ref<ZPagingRef<SaleOut>>() // 分页组件引用
...
/** 判断是否已选择 */
function isSelected(item: SaleOut) {
  return selectedRows.value.some(row => String(row.id) === String(item.id))
}

/** 切换选择状态 */
function toggleSelect(item: SaleOut) {
  if (isSelected(item)) {
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const params = {
      ...
      outTime: formatDateRange(queryParams.time),
      receiptEnable: true,
    }
