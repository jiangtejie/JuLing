<template>
  <view class="yd-page-container">
    <!-- 顶部导航栏 -->
    <wd-navbar title="其它入库详情" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />
    ...
    <!-- 底部操作按钮 -->
    <view v-if="canUpdate || canUpdateStatus || hasAccessByCodes(['erp:stock-in:delete'])" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button
          v-if="hasAccessByCodes(['erp:stock-in:delete'])"
...
const formData = ref<StockIn>() // 详情数据
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await buildErpDocumentDetail(await getStockIn(props.id), 'stock-in') as StockIn
  } finally {
    toast.close()
  }
}
...
async function handleDelete() {
  ...
  try {
    await dialog.confirm({ title: '提示', msg: '确定要删除该其它入库吗？' })
  } catch {
    return
  }
  deleting.value = true
...
/** 卸载 */
onUnload(() => {
  uni.$off('erp:stock-in:reload', getDetail)
})
</script>
