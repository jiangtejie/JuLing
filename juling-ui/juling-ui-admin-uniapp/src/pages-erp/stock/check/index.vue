<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="库存盘点" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />
          ...
          <view>
            <view class="p-24rpx">
              ...
              <view class="yd-text-sub mb-12rpx text-28rpx">
                <text class="yd-text-hint mr-8rpx">盘点时间：</text>{{ formatDateTime(item.checkTime) || '-' }}
              </view>
              <view class="yd-text-sub mb-12rpx flex text-28rpx">
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">盈亏数量：</text>{{ formatCount(item.totalCount) }}
                </view>
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">盈亏金额：</text>{{ formatMoney(item.totalPrice) }}
                </view>
              </view>
    ...
    <!-- 新增按钮 -->
    <wd-fab v-if="hasAccessByCodes(['erp:stock-check:create'])" position="right-bottom" type="primary" :expandable="false" @click="handleAdd" />
  </view>
</template>
...
import { getStockCheckPage } from '@/api/erp/stock/check'
...
const list = ref<StockCheck[]>([]) // 列表数据
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getStockCheckPage({ ...queryParams.value, pageNo, pageSize })
...
/** 初始化 */
onMounted(() => {
  uni.$on('erp:stock-check:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('erp:stock-check:reload', reload)
})
</script>
