<template>
  <view class="yd-page-container">
    <!-- 顶部导航栏 -->
    <wd-navbar title="库存盘点详情" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />

    <!-- 详情内容 -->
    <scroll-view class="min-h-0 flex-1" scroll-y scroll-with-animation>
      <wd-cell-group border>
        <wd-cell title="盘点单号" :value="formData?.no || '-'" />
        <wd-cell title="盘点时间" :value="formatDateTime(formData?.checkTime) || '-'" />
      ...
      <view v-if="items.length > 0" class="mt-24rpx">
        ...
        <view class="px-24rpx">
          ...
          >
            ...
            <view class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">账面库存：</text>
              <text class="min-w-0 flex-1">{{ formatCount(item.stockCount) }}</text>
            </view>
            <view class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">实际库存：</text>
              <text class="min-w-0 flex-1">{{ formatCount(item.actualCount) }}</text>
            </view>
            <view class="yd-text-sub mb-10rpx flex text-26rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">盈亏数量：</text>
...
const formData = ref<StockCheck>() // 详情数据
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await buildErpDocumentDetail(await getStockCheck(props.id), 'stock-check') as StockCheck
  } finally {
    toast.close()
  }
}
...
async function handleDelete() {
  ...
  try {
    await dialog.confirm({ title: '提示', msg: '确定要删除该库存盘点吗？' })
  } catch {
    return
  }
  deleting.value = true
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await dialog.confirm({ title: '提示', msg: `确定要${actionName}该库存盘点吗？` })
  } catch {
    return
  }
  statusLoading.value = true
  try {
    await updateStockCheckStatus(props.id, status)
...
/** 卸载 */
onUnload(() => {
  uni.$off('erp:stock-check:reload', getDetail)
})
</script>
