<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="其它出库" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />

    <!-- 搜索组件 -->
    <SearchForm @search="handleQuery" @reset="handleReset" />

    <!-- 其它出库列表 -->
    <z-paging ref="pagingRef" v-model="list" :fixed="false" class="min-h-0 flex-1" :default-page-size="10" :refresher-enabled="true" :inside-more="true" :loading-more-default-as-loading="true" empty-view-text="暂无其它出库数据" @query="queryList">
      <view class="p-24rpx">
        <view
          ...
          <view>
            <view class="p-24rpx">
              ...
              <view class="yd-text-sub mb-12rpx flex text-28rpx">
                ...
                <view class="flex-1">
                  <text class="yd-text-hint mr-8rpx">金额：</text>{{ formatMoney(item.totalPrice) }}
                </view>
              </view>
    ...
    <!-- 新增按钮 -->
    <wd-fab v-if="hasAccessByCodes(['erp:stock-out:create'])" position="right-bottom" type="primary" :expandable="false" @click="handleAdd" />
  </view>
</template>
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getStockOutPage({ ...queryParams.value, pageNo, pageSize })
...
/** 初始化 */
onMounted(() => {
  uni.$on('erp:stock-out:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('erp:stock-out:reload', reload)
})
</script>
