<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="设备分组" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />
...
import { getDeviceGroupPage } from '@/api/iot/device/group'
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getDeviceGroupPage({ ...queryParams.value, pageNo, pageSize })
...
/** 新增设备分组 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-iot/device/group/form/index' })
}
...
/** 初始化 */
onMounted(() => {
  uni.$on('iot:device-group:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('iot:device-group:reload', reload)
})
</script>
