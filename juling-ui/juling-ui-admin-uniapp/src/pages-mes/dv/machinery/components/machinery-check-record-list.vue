      ...
      <view class="px-24rpx py-8rpx pb-160rpx">
        ...
        >
          ...
          <view class="yd-text-sub mb-8rpx flex text-26rpx">
            <text class="yd-text-hint mr-8rpx shrink-0">点检时间：</text>
            <text class="min-w-0 flex-1 truncate">{{ formatDateTime(item.checkTime) || '-' }}</text>
          </view>
          <view class="yd-text-sub mb-8rpx flex text-26rpx">
            <text class="yd-text-hint mr-8rpx shrink-0">点检人：</text>
...
const list = ref<DvCheckRecord[]>([]) // 点检记录
const total = ref(0) // 总条数
const pagingRef = ref<ZPagingRef<DvCheckRecord>>() // 分页组件引用
...
/** 查看点检记录详情 */
function handleDetail(item: DvCheckRecord) {
  if (!item.id) {
    return
  }
  uni.navigateTo({ url: `/pages-mes/dv/checkrecord/detail/index?id=${item.id}` })
}
