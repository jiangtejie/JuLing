<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="出货检验单（OQC）" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />
      ...
      <view class="p-24rpx">
        ...
        >
          ...
          <view class="yd-text-sub mb-12rpx flex items-center text-28rpx">
            ...
            <text class="min-w-0 flex-1 truncate">{{ item.clientNickname || '-' }}</text>
          </view>
          <view v-if="item.batchCode" class="yd-text-sub mb-12rpx flex items-center text-28rpx">
          ...
          <view class="yd-text-sub mb-12rpx flex items-center text-28rpx">
            ...
            <text class="min-w-0 flex-1 truncate">
              发货 {{ item.outQuantity ?? '-' }}，检测 {{ item.checkQuantity ?? '-' }}，不合格 {{ item.unqualifiedQuantity ?? '-' }}
            </text>
          </view>
          <view class="yd-text-sub mb-12rpx flex items-center text-28rpx">
          ...
          <view class="yd-text-sub mb-12rpx text-28rpx">
            <text class="yd-text-hint mr-8rpx">出货日期：</text>{{ formatDateTime(item.outDate) || '-' }}
          </view>
          <view class="yd-text-sub mb-12rpx text-28rpx">
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['mes:qc-oqc:create'])"
...
import { getOqcPage } from '@/api/mes/qc/oqc'
...
const list = ref<QcOqc[]>([]) // 列表数据
const pagingRef = ref<ZPagingRef<QcOqc>>() // 分页组件引用
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getOqcPage({ ...queryParams.value, pageNo, pageSize })
...
/** 新增 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-mes/qc/oqc/form/index' })
}

/** 查看详情 */
function handleDetail(item: QcOqc) {
  uni.navigateTo({ url: `/pages-mes/qc/oqc/detail/index?id=${item.id}` })
}

/** 初始化 */
onMounted(() => {
  uni.$on('mes:qc:oqc:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('mes:qc:oqc:reload', reload)
})
</script>
