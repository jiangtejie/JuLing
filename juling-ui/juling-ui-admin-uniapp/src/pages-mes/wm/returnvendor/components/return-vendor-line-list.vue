    ...
    >
      <view class="px-24rpx py-8rpx">
        ...
        >
          ...
          <view class="yd-text-sub mb-8rpx flex text-26rpx">
            <text class="yd-text-hint mr-8rpx shrink-0">退货数量：</text>
        ...
        <view class="p-24rpx">
          ...
          >
            ...
            <view class="yd-text-sub mb-10rpx text-26rpx">
              <text class="yd-text-hint">物料：</text>{{ batch.itemCode || '-' }} {{ batch.itemName || '' }}
            </view>
            <view class="yd-text-sub mb-10rpx text-26rpx">
              <text class="yd-text-hint">供应商：</text>{{ batch.vendorName || '-' }}
            </view>
            <view class="yd-text-sub text-26rpx">
              <text class="yd-text-hint">入库日期：</text>{{ formatDate(batch.receiptDate) || '-' }}
            </view>
          </view>
        </view>
      </z-paging>
...
const formTitle = computed(() => formData.value.id ? '修改供应商退货单行' : '添加供应商退货单行') // 表单标题
const detailFormTitle = computed(() => detailFormData.value.id ? '编辑拣货明细' : '添加拣货明细') // 拣货明细表单标题
...
const detailSelectedItemText = computed(() => { // 拣货物料回显文本
...
const selectedStockText = computed(() => { // 库存回显文本
