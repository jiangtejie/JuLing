<template>
  <view class="mt-24rpx bg-white">
    ...
    <z-paging
      ...
      loading-more-no-more-text="没有更多发料物料了"
      empty-view-text="暂无发料物料"
      @query="queryList"
    >
      <view class="px-24rpx py-8rpx">
        ...
        >
          ...
          <view class="yd-text-sub mb-8rpx flex text-26rpx">
            <text class="yd-text-hint mr-8rpx shrink-0">发料数量：</text>
          ...
          <view v-if="stockMode" class="yd-bg-subtle mt-16rpx rounded-12rpx p-16rpx">
      ...
      <scroll-view class="min-h-0 flex-1" scroll-y>
        <wd-form ref="formRef" :model="formData" :schema="formSchema">
          <wd-cell-group border>
            ...
            />
            <wd-form-item title="发料数量" title-width="220rpx" prop="quantity" center>
      ...
      <scroll-view class="min-h-0 flex-1" scroll-y>
        <wd-form ref="detailFormRef" :model="detailFormData" :schema="detailFormSchema">
          <wd-cell-group border>
            ...
            <wd-form-item title="发料仓库" title-width="220rpx" prop="warehouseId">
  ...
  <!-- 库存选择弹窗 -->
  <MaterialStockPicker ref="stockPickerRef" :multiple="false" positive-only @confirm="handleStockConfirm" />
</template>
...
import type { WmOutsourceIssueDetail } from '@/api/mes/wm/outsourceissue/detail'
import type { WmOutsourceIssueLine } from '@/api/mes/wm/outsourceissue/line'
...
const list = ref<WmOutsourceIssueLine[]>([]) // 发料物料列表
const pagingRef = ref<ZPagingRef<WmOutsourceIssueLine>>() // 分页组件引用
const formVisible = ref(false) // 行表单显示状态
const formLoading = ref(false) // 表单提交状态
const formRef = ref<FormInstance>() // 表单引用
const formData = ref<WmOutsourceIssueLine>(getDefaultFormData()) // 表单数据
...
const detailFormData = ref<WmOutsourceIssueDetail>(getDefaultDetailFormData()) // 拣货明细表单数据
const currentDetailLine = ref<WmOutsourceIssueLine>() // 当前拣货明细所属行
const itemPickerRef = ref<InstanceType<typeof ItemPicker>>() // 物料选择器引用
const detailListMap = ref<Record<number, WmOutsourceIssueDetail[]>>({}) // 拣货明细列表
...
const formTitle = computed(() => formData.value.id ? '编辑发料物料' : '添加发料物料')
...
const formSchema = createFormSchema({
  ...
  quantity: [
    { required: true, message: '发料数量不能为空' },
    { validator: value => Number(value) > 0 || '发料数量必须大于 0' },
  ],
})
...
/** 默认表单数据 */
function getDefaultFormData(): WmOutsourceIssueLine {
  return {
...
/** 默认拣货明细表单数据 */
function getDefaultDetailFormData(): WmOutsourceIssueDetail {
  return {
    issueId: props.issueId,
  }
}
...
async function queryList(pageNo: number, pageSize: number) {
  ...
  try {
    const data = await getOutsourceIssueLinePage({
      pageNo,
      pageSize,
      issueId: props.issueId,
    })
...
async function loadDetailList(lineId?: number) {
  ...
  try {
    const data = await getOutsourceIssueDetailListByLineId(lineId)
...
/** 打开编辑表单 */
function openUpdateForm(item: WmOutsourceIssueLine) {
  formData.value = { ...item }
  formVisible.value = true
}
...
async function handleSubmit() {
  ...
  try {
    if (formData.value.id) {
      await updateOutsourceIssueLine(formData.value)
      toast.success('修改成功')
    } else {
      await createOutsourceIssueLine(formData.value)
      toast.success('添加成功')
    }
...
/** 删除发料物料 */
async function handleDelete(item: WmOutsourceIssueLine) {
  try {
  ...
  await deleteOutsourceIssueLine(item.id)
  toast.success('删除成功')
  reload()
}

/** 打开新增拣货明细表单 */
function openCreateDetailForm(item: WmOutsourceIssueLine) {
...
/** 打开编辑拣货明细表单 */
async function openUpdateDetailForm(item: WmOutsourceIssueLine, detail: WmOutsourceIssueDetail) {
  currentDetailLine.value = item
...
/** 删除拣货明细 */
async function handleDeleteDetail(detail: WmOutsourceIssueDetail) {
  try {
    await dialog.confirm({
      title: '提示',
  ...
  await deleteOutsourceIssueDetail(detail.id)
  toast.success('删除成功')
...
async function handleSubmitDetail() {
  ...
  try {
    if (detailFormData.value.id) {
      await updateOutsourceIssueDetail(detailFormData.value)
      toast.success('修改成功')
    } else {
      await createOutsourceIssueDetail(detailFormData.value)
      toast.success('新增成功')
    }
...
/** 初始化 */
onMounted(() => {
  uni.$on('mes:wm:outsourceissue:reload', reload)
})
...
/** 卸载 */
onUnmounted(() => {
  uni.$off('mes:wm:outsourceissue:reload', reload)
})
</script>
