    ...
    <scroll-view class="min-h-0 flex-1" scroll-y>
      <wd-form ref="formRef" :model="formData" :schema="formSchema">
        <wd-cell-group border>
          <wd-form-item title="页面名称" title-width="200rpx" prop="name">
            <wd-input v-model="formData.name" clearable placeholder="请输入页面名称" />
          </wd-form-item>
      ...
      <!-- 装修属性说明 -->
      <view class="yd-text-hint px-24rpx py-18rpx text-24rpx">
        移动端仅维护页面基础信息与 JSON 装修属性，可视化拖拽装修请前往 PC 端。
      </view>
    </scroll-view>
...
const getTitle = computed(() => props.id ? '编辑装修页面' : '新增装修页面')
const formLoading = ref(false) // 表单提交状态
const formData = ref<DiyPageFormData>({
  id: undefined,
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.id) {
      await updatePromotionDiyPage(baseData)
      // 装修属性走独立接口更新
      ...
      await createPromotionDiyPage(baseData)
      toast.success('新增成功')
    }
