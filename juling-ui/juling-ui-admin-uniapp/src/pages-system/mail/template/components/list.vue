      ...
      <view class="p-24rpx">
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="yd-text-sub mb-12rpx flex items-center text-28rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">发送人名称：</text>
              <text class="min-w-0 flex-1 truncate">{{ item.nickname }}</text>
            </view>
            <view class="yd-text-sub mb-12rpx flex items-center text-28rpx">
              <text class="yd-text-hint mr-8rpx shrink-0">标题：</text>
...
const list = ref<MailTemplate[]>([]) // 列表数据
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getMailTemplatePage(params)
