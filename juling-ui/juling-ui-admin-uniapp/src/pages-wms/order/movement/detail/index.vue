  ...
  <view class="yd-page-container pb-120rpx">
    ...
    <view>
      <wd-cell-group border title="单据信息">
        <wd-cell title="移库单号" :value="formData?.no || '-'" />
        <wd-cell title="单据状态">
          <dict-tag :type="DICT_TYPE.WMS_ORDER_STATUS" :value="formData?.status" />
        </wd-cell>
        <wd-cell title="来源仓库" :value="formData?.sourceWarehouseName || '-'" />
        <wd-cell title="目标仓库" :value="formData?.targetWarehouseName || '-'" />
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="canUpdate && hasAccessByCodes(['wms:movement-order:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="canUpdate && hasAccessByCodes(['wms:movement-order:complete'])"
...
interface DetailRow extends MovementOrderDetail {
  totalPrice?: number
}
...
const formData = ref<MovementOrder>() // 详情数据
...
const moreActions = computed(() => {
  const actions = []
  if (canDelete.value && hasAccessByCodes(['wms:movement-order:delete'])) {
    actions.push({ name: '删除', value: 'delete' })
  }
  if (canUpdate.value && hasAccessByCodes(['wms:movement-order:cancel'])) {
...
async function getDetail() {
  ...
  formData.value = await getMovementOrder(Number(props.id))
}

/** 编辑移库单 */
function handleEdit() {
  uni.navigateTo({
    url: `/pages-wms/order/movement/form/index?id=${props.id}`,
  })
}
...
async function handleComplete() {
  ...
  try {
    await completeMovementOrder(Number(props.id))
    toast.success('移库成功')
    uni.$emit('wms:movement-order:reload')
    await getDetail()
  } finally {
...
async function handleCancel() {
  ...
  actionLoading.value = true
  try {
    await cancelMovementOrder(Number(props.id))
    toast.success('作废成功')
    uni.$emit('wms:movement-order:reload')
    await getDetail()
  } finally {
