    ...
    <view class="rounded-32rpx bg-white px-22rpx pb-18rpx pt-20rpx shadow-[0_12rpx_48rpx_rgba(0,0,0,0.08)]">
      <view v-if="attachmentUrls.length" class="mb-14rpx flex flex-wrap gap-10rpx">
        <view
          ...
          :key="url"
          class="yd-bg-page max-w-full flex items-center gap-8rpx rounded-full px-14rpx py-8rpx"
        >
          <wd-icon name="file" size="24rpx" color="#1677ff" />
          <text class="yd-text-sub max-w-360rpx truncate text-22rpx">{{ getFileNameFromUrl(url) || '附件' }}</text>
      ...
      <view class="mt-14rpx flex items-center justify-between">
        <view class="flex items-center gap-12rpx">
          <view
            class="yd-bg-page relative h-54rpx w-54rpx flex items-center justify-center rounded-full"
          ...
          <view
            ...
            :class="enableContext ? 'yd-bg-info-soft yd-text-link' : 'yd-bg-page yd-text-sub'"
          ...
          <view
            class="rounded-full px-18rpx py-10rpx text-23rpx"
            :class="enableWebSearch ? 'yd-bg-info-soft yd-text-link' : 'yd-bg-page yd-text-sub'"
    ...
    >
      <view class="bg-white px-28rpx pb-[calc(32rpx+env(safe-area-inset-bottom))] pt-24rpx">
        <view class="mb-24rpx flex items-center justify-between">
          <view>
            <view class="yd-text-main text-31rpx font-semibold">
              添加附件
            </view>
