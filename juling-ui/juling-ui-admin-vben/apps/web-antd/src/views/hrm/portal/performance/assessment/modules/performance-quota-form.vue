...
import { Alert, Button, Empty, message, Spin } from 'ant-design-vue';
...
import PerformanceQuotaGrid from './performance-quota-grid.vue';
...
const [Drawer, drawerApi] = useVbenDrawer({
  async onOpenChange(isOpen) {
    if (!isOpen) return;
    const assessmentId = (drawerApi.getData() as { assessmentId?: number })
      ?.assessmentId;
    if (!assessmentId) {
...
<template>
  <Drawer class="w-[960px]" title="制定绩效指标">
