...
interface TinymacProps {
  ...
  /** 兼容表单以 value 绑定（vben form baseModelPropName=value） */
  value?: string;
}
...
/** 内容来源：表单以 value 绑定时优先 value，否则走 modelValue */
const displayValue = computed(() =>
  props.value === undefined ? modelValue.value : props.value,
);
...
/** 编辑器内容变化：同步内部状态，并转发给外层（v-model 或表单 value 绑定） */
function handleModelUpdate(val: string) {
  modelValue.value = val;
  const eventKey = 'onUpdate:value';
  const handler = attrs[eventKey];
