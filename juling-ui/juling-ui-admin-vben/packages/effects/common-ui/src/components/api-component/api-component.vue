...
// 同步外层以 `modelPropName`（如 value）绑定的值到内部状态，避免组件重置后内部值残留旧数据
watch(
  () => attrs[props.modelPropName as keyof typeof attrs],
  (val) => {
    // 仅当外层确实以该 prop 绑定时才同步（modelValue 默认路径由 defineModel 自行处理）
    if (props.modelPropName === 'modelValue' || !(props.modelPropName in attrs)) {
      return;
    }
    if (!isEqual(val, unref(modelValue))) {
