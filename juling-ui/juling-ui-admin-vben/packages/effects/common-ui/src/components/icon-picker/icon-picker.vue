...
// 外层若按 modelValueProp（如表单的 value）绑定，则同步到内部状态，保证回显正确
watch(
  () =>
    props.modelValueProp === 'modelValue'
      ? undefined
      : attrs[props.modelValueProp as keyof typeof attrs],
  (val) => {
    if (props.modelValueProp !== 'modelValue' && typeof val === 'string') {
...
const handleClick = (icon: string) => {
  // 统一走 updateCurrentSelect：内部状态 + 按 modelValueProp 转发外层（兼容 v-model / 表单 value 两种绑定）
