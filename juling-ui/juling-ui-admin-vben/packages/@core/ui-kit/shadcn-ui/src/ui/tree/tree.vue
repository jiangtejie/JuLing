...
/** 表单（baseModelPropName=value）以 value 绑定时，比较两个候选值是否一致（内容级比较） */
...
// 外部以 value（vben 表单绑定）提供值时，同步到内部 modelValue，保证回显正确
watch(
  () => ('value' in attrs ? attrs.value : undefined),
  (val) => {
    if (!('value' in attrs)) {
      return;
    }
    const next = val as Arrayable<number | string> | undefined;
    if (isSameValue(next, modelValue.value)) {
      return;
    }
...
// 内部值变化时，若外层监听 onUpdate:value（表单 value 绑定），原样转发，避免选择不落表单
watch(
  () => modelValue.value,
  (val) => {
    if (!('value' in attrs)) {
    ...
    const handler = attrs['onUpdate:value'];
