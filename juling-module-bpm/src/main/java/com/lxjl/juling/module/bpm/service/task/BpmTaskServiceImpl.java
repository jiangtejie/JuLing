...
import com.lxjl.juling.module.bpm.controller.admin.task.vo.task.*;
import com.lxjl.juling.module.bpm.convert.task.BpmTaskConvert;
...
import com.lxjl.juling.module.bpm.enums.definition.*;
import com.lxjl.juling.module.bpm.enums.task.*;
...
import com.lxjl.juling.module.bpm.service.comment.BpmCommentService;
import com.lxjl.juling.module.bpm.service.definition.BpmFormService;
import com.lxjl.juling.module.bpm.service.definition.BpmModelService;
...
import static com.lxjl.juling.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    public List<UserTask> getUserTaskListByReturn(String id) {
        ...
        // 2.3 过滤：只能退回到已经处理过的节点（排除审批未经过的节点）。相关 issue：上游仓库
    ...
    @DataPermission(enable = false) // 关闭数据权限，避免查询不到用户数据。相关案例：上游仓库
    ...
    private Map<String, Object> validateAndSetNextAssignees(String taskDefinitionKey, Map<String, Object> variables, BpmnModel bpmnModel,
        ...
        for (UserTask nextFlowNode : nextFlowNodes) {
            ...
            if (ObjUtil.equals(candidateStrategy, BpmTaskCandidateStrategyEnum.APPROVE_USER_SELECT.getStrategy())) {
                // 特殊：如果当前节点已经存在审批人，则不允许覆盖。 例如并行节点后，设置的审批人自选节点。 上游社区讨论
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 juling-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void returnTask(Long userId, BpmnModel bpmnModel, Task currentTask, FlowElement targetElement, BpmTaskReturnReqVO reqVO) {
        ...
        //    相关 issue：上游仓库
        // ③ moveActivityIdsToSingleActivityId 使用遇到问题， 相关 issue 上游仓库
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：上游仓库
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：上游仓库
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://github.com/jiangtejie/JuLing">issue</a> 反馈
             */
            @Override
