package com.lxjl.juling.module.iot.framework.job.core;
...
public class IotSchedulerManager {
    ...
    public IotSchedulerManager(DataSource dataSource,
        ...
        // 2.2.1 【本地化改造】PostgreSQL 需要显式指定 PostgreSQLDelegate，否则集群 checkin 时
        // 会出现 invalid input syntax for type bigint 报错（框架只为 spring.quartz 主调度器自动配置）
        try (Connection connection = dataSource.getConnection()) {
            ...
            log.warn("[constructor][获取数据库类型失败，IoT Scheduler 将使用 Quartz 默认 DriverDelegate]", e);
        }
        // 2.3 线程池相关配置
