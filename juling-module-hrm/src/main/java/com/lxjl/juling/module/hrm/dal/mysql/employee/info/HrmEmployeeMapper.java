...
public interface HrmEmployeeMapper extends BaseMapperX<HrmEmployeeDO> {
    ...
    private void appendTodoType(MPJLambdaWrapperX<HrmEmployeeDO> query, HrmEmployeePageReqVO reqVO) {
        ...
        if (HrmEmployeeTodoTypeEnum.PENDING_LEAVE.getType().equals(todoType)) {
            ...
            // 用 EXTRACT 取月份/日：MySQL 与 PostgreSQL 均支持（MONTH()/DAYOFMONTH() 为 MySQL 专有，
            // 在 PostgreSQL 下会报 function month(timestamp) does not exist）
            query.apply("EXTRACT(MONTH FROM birthday) = {0}", now.getMonthValue());
            if (now.getMonthValue() == 2 && !now.isLeapYear()) {
                // 非闰年不存在 2 月 29 日，与首页日历和年龄计算保持一致。
                query.apply("EXTRACT(DAY FROM birthday) <> 29");
            }
        }
    }
