package com.lxjl.juling.framework.excel.core.handler;
...
public class SelectSheetWriteHandler implements SheetWriteHandler {
    ...
    private void getSelectDataList(int colIndex, Field field) {
        ...
        if (StrUtil.isNotEmpty(dictType)) { // 情况一： 字典数据 （默认）
            List<String> dictDataLabels = DictFrameworkUtils.getDictDataLabelList(dictType);
            // 字典项为空（字典未配置数据）时不生成下拉：否则会生成 字典sheet!$F$1:$F$0 这样的反向区间，
            // POI 解析公式会抛 FormulaParseException，导致整个导入模板下载失败
            if (CollUtil.isNotEmpty(dictDataLabels)) {
                selectMap.put(colIndex, dictDataLabels);
            }
            return;
        }
        ...
        if (CollUtil.isNotEmpty(options)) {
            selectMap.put(colIndex, options);
        }
    }
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        // 过滤空下拉项，避免生成 $F$1:$F$0 这种反向区间（见 getSelectDataList 的说明）
